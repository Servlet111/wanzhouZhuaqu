...
@Controller
@Component
public class GovPubSc {
    private int Cid = 0; // 将Cid声明为成员变量
    @Autowired
    GovPubService govPubService;
    public void scrapeGovPub() {
        ...
        String baseUrl = "http://www.wz.gov.cn/jkq/wzjkq/zwgk_106889";
        //信息公开的基础 URL
        String inforPubUrl = "http://www.wz.gov.cn/jkq/wzjkq/zwgk_106889/gsgg/";
        // 政策信息的基础 URL
        String policyUrl = "http://www.wz.gov.cn/jkq/wzjkq/zwgk_106889/zcwj_106892/";
        // 政府信息公开指南 URL
        String govGkUrl = "http://www.wz.gov.cn/jkq/wzjkq/zwgk_106889/gkzn/";
        // 政府信息年报 URL
        String anGovGkUrl = "http://www.wz.gov.cn/jkq/wzjkq/xwzx/qydt/";
        List<GovPubDao> dataList = new ArrayList<>();

        try {
            //信息公开1
            // 连接政府信息的 URL 并解析页面内容
            Document inforPub = Jsoup.parse(new URL(inforPubUrl), 30000);
            String inforPubHtmlContent = inforPub.html();
            // 使用正则表达式匹配createPage函数调用
            Pattern pattern = Pattern.compile("createPage\\((\\d+)");
            Matcher matcher = pattern.matcher(inforPubHtmlContent);
            while (matcher.find()) {
                //是用来表示是企业还是政府govBid
                int infoBid = 1;
                ...
                for (int i = 0; i <=param1Int; i++) {
                    String infoNewUrl;
                    if (i == 0) {
                        infoNewUrl = inforPubUrl + "index.html"; // 当 i 为0时使用这个连接
                    } else {
                        infoNewUrl = inforPubUrl + "index_" + i + ".html";
                    }
                    /*curretbid用来记数*/
                    extractInformation(inforPubUrl,dataList,infoNewUrl, "信息公开", infoBid);
                }

            }
            //政策信息2
            // 连接政策信息 的 URL 并解析页面内容
            Document publicyDoc = Jsoup.parse(new URL(policyUrl), 30000);
            String publicyHtmlContent = publicyDoc.html();

            // 重新使用正则表达式匹配createPage函数调用

            Matcher matcher2 = pattern.matcher(publicyHtmlContent);
            ...
            while (matcher2.find()) {
                ...
                for (int i = 0; i <=param2Int; i++) {
                    /*corpBid表示是企业信息id*/
                    String publicyNewUrl;
                    if (i == 0) {
                        publicyNewUrl = policyUrl + "index.html"; // 当 i 为0时使用这个连接
                    } else {
                        publicyNewUrl = policyUrl + "index_" + i + ".html";
                    }
                    /*currentid用来及数字*/
                    extractInformation(policyUrl,dataList,publicyNewUrl, "政策信息", publicyBid);
                }

            }
            //政府信息公开指南3
            // 政府信息公开指南 的 URL 并解析页面内容
            Document govGkDoc = Jsoup.parse(new URL(govGkUrl), 30000);
            String  govGkHtmlContent = govGkDoc.html();
            // 重新使用正则表达式匹配createPage函数调用
            Matcher matcher3 = pattern.matcher(govGkHtmlContent);
            while (matcher3.find()) {
                int govGkBid= 3;
                ...
                for (int i = 0; i <=param2Int; i++) {
                    ...
                    String govGkNewUrl;
                    if (i == 0) {
                        govGkNewUrl = govGkUrl + "index.html"; // 当 i 为0时使用这个连接
                    } else {
                        govGkNewUrl = govGkUrl + "index_" + i + ".html";
                    }
                    /*currentid用来及数字*/
                    extractInformation(policyUrl,dataList,govGkNewUrl, "政府信息公开指南", govGkBid);
                }
            ...
            //政府信息年报 4
            // 连接政策信息 的 URL 并解析页面内容
            Document angovGkDoc = Jsoup.parse(new URL(anGovGkUrl), 30000);
            String anGovGkHtmlContent = angovGkDoc.html();
            Matcher matcher4 = pattern.matcher(anGovGkHtmlContent);
            while (matcher4.find()) {
                int anGovGkBid= 4;
                ...
                for (int i = 0; i <=param2Int; i++) {
                    ...
                    /*currentid用来及数字*/
                    extractInformation(policyUrl,dataList,publicyNewUrl, "政府信息年报", anGovGkBid);
                }
        ...
        govPubService.saveBatch(dataList);
        System.out.println("数据插入成功！");
    }

    private int extractInformation(String baseUrl, List<GovPubDao> dataList, String NewUrl, String category, int bid) throws IOException {
...
// 处理状态码
            ...
            GovPubDao govPubDao = new GovPubDao();
            govPubDao.setId(4);
            govPubDao.setName("政务公开");
            govPubDao.setBid(bid);
            govPubDao.setBname(category);
            govPubDao.setCid(Cid);
            govPubDao.setCname(title);
            govPubDao.setCtime(date);
            govPubDao.setCurl(fullLink);
            govPubDao.setDname(title);
            govPubDao.setDyuan(csource);
            govPubDao.setNeirong(articleContent);

            dataList.add(govPubDao);
        }
